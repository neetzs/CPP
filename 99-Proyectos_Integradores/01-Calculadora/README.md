...
## Concepto Clave
Una calculadora completa en C++ que implementa operaciones básicas y avanzadas, con manejo de expresiones matemáticas, funciones trigonométricas, logaritmos, y una interfaz de usuario en consola. El proyecto integra conceptos de POO, manejo de errores, templates y algoritmos.
...
class EvaluadorExpresiones {
    // Maneja el parsing y evaluación de expresiones matemáticas
};
...
class OperacionMatematica {
    // Representa una operación matemática con sus operandos y resultado
};
```
...
private:
    ...
    map<char, pair<int, function<double(double, double)>>> operadoresBinarios = {
        ...
        {'%', {2, [](double a, double b) { return fmod(a, b); }}}
    };
        ...
        for (size_t i = 0; i < expresion.length(); ++i) {
            ...
            if (isdigit(c) || c == '.' || (c == '-' && (i == 0 || expresion[i-1] == '(' || esOperador(expresion[i-1])))) {
                ...
                if (c == '(') {
                    ...
                    if (!operadores.empty()) operadores.pop(); // Remover '('
    ...
    double evaluarPostfija(queue<string> expresionPostfija) {
        ...
        while (!expresionPostfija.empty()) {
            ...
            if (esNumero(token)) {
                ...
                if (pila.empty()) throw runtime_error("Faltan operandos para función");
                double operando = pila.top(); pila.pop();
                pila.push(funcionesUnarias[token](operando));
            } else if (esOperador(token[0])) {
                if (pila.size() < 2) throw runtime_error("Faltan operandos para operador");
        ...
        if (pila.size() != 1) throw runtime_error("Expresión mal formada");
        return pila.top();
    }
...
### Operaciones Básicas
- Suma, resta, multiplicación, división
- Potencias (^) y raíces cuadradas (sqrt)
- Operaciones módulo (%)
...
### Modos de Operación
- **Modo Básico**: Solo operaciones aritméticas simples
- **Modo Avanzado**: Funciones matemáticas y expresiones complejas

## Estructura de Archivos

```
calculadora/
├── calculadora.cpp      # Archivo principal
├── Calculadora.h        # Clase principal
├── EvaluadorExpresiones.h # Evaluador de expresiones
├── Historial.h          # Gestión del historial
├── OperacionMatematica.h # Representación de operaciones
...
- `ultimo` - Ver último resultado
- `modo` - Cambiar entre modos
...
1. **Preprocesamiento**: Separar funciones de paréntesis
2. **Tokenización**: Convertir expresión a tokens
3. **Conversión**: Infija → Postfija (Shunting-yard algorithm)
4. **Evaluación**: Procesar notación postfija con pila

### Manejo de Errores
- Divisiones por cero
- Funciones con dominios inválidos
- Expresiones mal formadas
- Paréntesis no balanceados

### Optimizaciones
- Evaluación eficiente usando pilas
- Historial con capacidad limitada
- Parsing optimizado de expresiones

## Extensiones Posibles

### Interfaz Gráfica
- Usar Qt o GTK para interfaz visual
...
### Conectividad
- Servidor HTTP para calculadora web
- API REST para integraciones
- Conexión con bases de datos

## Conclusión

Esta calculadora implementa un sistema completo de evaluación matemática con:

- ✅ **Arquitectura modular** con clases bien definidas
- ✅ **Manejo robusto de errores** con excepciones personalizadas
- ✅ **Interfaz de usuario intuitiva** con comandos claros
- ✅ **Funcionalidades avanzadas** como funciones trigonométricas
- ✅ **Historial persistente** de operaciones
- ✅ **Dos modos de operación** para diferentes niveles de usuario

El proyecto demuestra el uso efectivo de conceptos avanzados de C++ como templates, excepciones, contenedores STL y algoritmos para crear una aplicación completa y funcional.