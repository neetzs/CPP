...
## Concepto Clave
El **encapsulamiento** oculta los detalles internos de una clase y expone solo la interfaz necesaria.
...
class Coche {
private:
    Motor motor;      // Detalle interno
    Transmision trans;// Detalle interno

public:
    void acelerar();  // Interfaz simple
    void frenar();
};
```
...
public:
    ...
    void aumentarSalario(double porcentaje) {
        if (porcentaje > 0 && porcentaje <= 50) {  // Máximo 50%
...
### ✅ Seguridad
- Datos protegidos de modificación accidental
- Validación de datos
...
2. **Métodos**: `public` para interfaz, `private` para helpers
3. **Getters/Setters**: Para acceso controlado
4. **Validación**: En setters para mantener integridad
5. **No exponer**: No hacer públicos detalles de implementación
6. **Consistencia**: Mantener interfaz coherente
...
class MalaClase {
public:
    int datoInterno;  // Debería ser private
};
```
...
private:
    int dato;
public:
    int getDato() { return dato; }      // Sin validación
