...
private:
    int puertas;
public:
    void abrirPuertas() { cout << "Abriendo puertas" << endl; }
};
```
...
class Derivada : protected Base {
    // Miembros public de Base se vuelven protected
    // Miembros protected de Base siguen siendo protected
};
```
...
class Derivada : private Base {
    // Todos los miembros heredados se vuelven private
};
```
...
private:
    string nombre;
public:
    // Llamar constructor de base
    Derivada(int v, string n) : Base(v), nombre(n) {}
};
```
...
1. **Constructores**: Llama al constructor de la clase base primero
2. **Destructores**: Declara `virtual` en la clase base si hay herencia
3. **Métodos virtuales**: Usa `virtual` para permitir sobrescritura
4. **override**: Usa `override` para sobrescribir métodos virtuales
5. **Acceso**: `protected` permite acceso desde clases derivadas
6. **Herencia múltiple**: Evita cuando sea posible, puede ser compleja

## Ventajas de la Herencia

### ✅ Reutilización de Código
- No repetir código común
- Extender funcionalidad existente

### ✅ Jerarquía Lógica
- Relaciones "es-un" naturales
- Organización clara del código

### ✅ Polimorfismo
- Tratar objetos derivados como base
- Interfaces comunes
...
### Herencia Incorrecta
```cpp
class Coche : public Motor {  // ❌ Un coche NO es un motor
    // ...
};
```
...
```cpp
class Hijo : public Padre1, public Padre2 {  // Puede causar conflictos
    // ...
};
```

### Dependencia Fuerte
- Clases derivadas dependen de la implementación de la base
- Cambios en base afectan a derivadas